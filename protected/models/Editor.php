...
class Editor extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name_surname, username, password', 'required'),
			array('status', 'numerical', 'integerOnly'=>true),
			array('name_surname, email', 'length', 'max'=>45),
			array('username', 'length', 'max'=>20),
			array('password', 'length', 'max'=>32),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('editor_id, name_surname, username, password, email, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
            'relArticles'=>array(self::HAS_MANY, 'Article', 'editor_id'),
		);
	}
